...
class WxpayController extends Controller{
	...
	function get_user_info(){
        ...
        //	'secret'  => "59be388e1ebeecd6fdbdfca4b7003294",
          //  'js_code'    => "071NcV8N0mw8w62Tyy7N0Xjc9N0NcV8k",
		...
		/*$params = array(
            ...
            'secret'     => "477f55d8473f59129e8583bc303a979b",
            'js_code'    => $code,
            'grant_type' => 'authorization_code',
		 );*/
          ...
          //  $insertId = M('home_order','xxf_witkey_')->add($data);
            ...
            //   if($insertId){
        	 if(1){
                ...
                /* 调用微信【统一下单】 */
                $this->pay($data_total*100,$data['openid'],$data['crsNo'],$data['program_id']);
            }else{
                echo $insertId;
            }
            //echo json_encode($re);
    ...
    private function Pay($total_fee,$openid,$order_id,$program_id){
        if(empty($total_fee)){
        ...
        //会员支付信息
       // $members=M("members")->where("openid='".$openid."'")->find();
        
        //商户支付信息
        $program_id=M("user_info")->where("program_id='".$program_id."'")->find();
      //  $appid =        'wx22d43e5e8dc5955f';// 小程序appid  如果是公众号 就是公众号的appid;小程序就是小程序的appid
        $appid =        $program_id["appid"];
        $body =         '自己填';
      //  $mch_id =       '1490465072';
        $mch_id =       $program_id["shanghu_num"];
       //  $KEY = '051A9911DE7B5BBC610B76F4EDA834A0';//你申请微信支付的key
       $KEY = 			$program_id["pay_key"];
        $nonce_str =    rand(1111111111,9999999999);//随机字符串
        $notify_url =   'https://sz800800.com/Wxpay/xiao_notify_url';  //支付完成回调地址url,不能带参数
